...
def _launch_setup(context, *args, **kwargs):
    ...
    robot_cfg = os.path.join(share, "config", "robot.yaml")
    physics_launch = os.path.join(
        share, "launch", "gazebo_physics.launch.py"
    )
        ...
        TimerAction(period=0.5, actions=[
            ExecuteProcess(
                cmd=[
                    ...
                    "ros2 topic pub --once /posture_command "
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument(
            "urdf_path", default_value=_default_urdf_path()
        ),
