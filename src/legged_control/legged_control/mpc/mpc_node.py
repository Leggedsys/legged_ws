...
_STAIR_MIN_PERIOD = 1.2  # s — crawl cycle floor (4 swings per cycle)
_STAIR_SWING_RATIO = 0.2 # one leg at a time, 3 always planted (max 0.24)
...
class MPCNode(Node):
    ...
    def _compute_mpc_joints(self, now: float) -> JointCommand:
        ...
        if bool(self.get_parameter("stair_mode").value):
        ...
        stair = bool(self.get_parameter("stair_mode").value)
        if not self._walking:
            # phase offsets jump on a gait switch — only flip while standing
            self._gait.set_mode("crawl" if stair else "trot")
        if stair:
