...
Effort is forwarded as JOINT-frame torque (rotor tau × gear_ratio,
direction-corrected) — the contact-detection signal for stair climbing.
"""
...
class JointAggregatorNode(Node):
    ...
    def _publish(self) -> None:
        ...
        for name in self._names:
            ...
            gear_ratio = float(cfg.get("gear_ratio", 1.0))
            out.position.append(direction * float(entry["position"]) + zero_offset)
            out.velocity.append(direction * float(entry["velocity"]))
            # motor_bus reports ROTOR torque; joint torque scales with the
            # gear ratio and flips with the direction convention.
